...
abstract class User implements UserInterface {
	...
	public function login($login = NULL, $password = NULL) {
		...
		if ( $this->dao->exists($pointer)) {
			...
			$this->id       = $data[0]['admin_id'];
			$this->login    = $data[0]['admin_login'];
			$this->password = $data[0]['admin_password'];
			$this->level    = $data[0]['admin_level'];
			$this->name     = $data[0]['admin_name'];
			Session::insert( array('sid' => Validation::encriptSession( time() . $data[0]['admin_login'] . $data[0]['admin_password']), 'id' => $data[0]['admin_id'],'login' => $data[0]['admin_login'], 'level' => $data[0]['admin_level'], 'name' => $data[0]['admin_name']), 'user');
			return true;
		}
