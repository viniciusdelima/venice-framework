...
class ArrayHelper {
	...
	/**
	 * Formata um array de alias para um array de chave => valor válido a ser inserido no array de configurações.
	 * 
	 * @param array $arr
	 * @param boolea $additional
	 * @return array
	 */
	public static function formatArrayAlias(array $arr, $additional = false) {
		$result = array();
		$alias = '';
		if ( $additional ) {
			foreach ($arr as $k => $i) {
				...
				foreach ($i as $j => $l) {
					if ( is_array($l)) {
						...
						$result_info['module'] = $module;
						$alias = $l;
					}
				}
				$result[$alias] = $result_info; 
				$result = self::clean($result);
				$result[$alias]['routing'] = false;
			}
		}
		else {
			foreach ($arr as $k => $i) {
	...
	/**
	 * Aplica a função de anti_injection há um array.
	 * 
	 * @param array $arr
	 * @return array
	 */
	public static function escapeArray($arr) {
		foreach ($arr as $k => $i) {
			$arr[$k] = Validation::antiInjection($i);
		}
		return $arr;
	}
