...
class RegisterFieldDAO implements DAOInterface {
	...
	public function insert($Field) {
		if ($Field instanceof Field) {
			...
			try {
				...
				$query = 'INSERT INTO pd_register_fields SET field_name=?, field_type=?, field_description=?, field_mandatory=?, field_active=?';
				$new = array();
				$i = 0;
	...
	public function get($Field) {
		...
		try {
			$DB = DBFactory::getInstance();
			$data = $DB->get('SELECT * FROM pd_register_fields WHERE field_id=?', array($id));
			return $data;
		}
		catch (DBException $e) {
			throw $e;
	...
	public function getAll($Field) {
		if ($Field instanceof Field) {
			try {
				$DB = DBFactory::getInstance();			
				$query = 'SELECT * FROM pd_register_fields';
				$fields = $DB->get($query);
				return $fields;
			}
			catch (DBException $e) {
